...
void ekf_propagator_test(slate_t *slate) {
    ...
    for (int i = 0; i < steps; i++) {
        ...
        // update measurement vectors according to ground truth
        b_gyro_drift = b_gyro_drift + IMU_DRIFT_VARIANCE * sqrtf(dt) * random_float3(); // add random walk to gyro
        slate->w_body = w_body + b_gyro_drift;
        slate->sun_vector_body = qrot(q_eci2body, slate->sun_vector_eci);
        slate->b_body = qrot(q_eci2body, slate->b_body);
