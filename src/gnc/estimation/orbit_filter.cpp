        ...
        orbit_filter_t &orbit_filter)
{
    ...
    imu.a_body = {0.0f, 0.0f, 0.0f};

    // Initialize position from GPS (LLA . ECEF . ECI)
    float3 r_ecef = lla_to_ecef(gps.gps_lat, gps.gps_lon, gps.gps_alt);
    gps.r_ecef = r_ecef;
    gps.r_eci = ecef_to_eci(r_ecef, gps.MJD);

    // Initialize velocity from GPS speed/course (ENU . ECEF . ECI)
    float3 v_enu = {0.0f,0.0f,0.0f};
    float3 lla = {gps.gps_lat, gps.gps_lon, gps.gps_alt};
    float3 v_ecef = enu_to_ecef(v_enu, lla);
    gps.v_eci = ecef_to_eci(v_ecef, gps.MJD);
    ...
    for (int i = 0; i < 6 * 6; i++)
    {
        if (i % 7 == 0 && i < 18)
        {
            orbit_filter.P_orbit[i] = INIT_POSITION_VARIANCE;
        }
        else if (i % 7 == 0 && i >= 18)
        {
            orbit_filter.P_orbit[i] = INIT_VELOCITY_VARIANCE;
        }
        else
        {
            orbit_filter.P_orbit[i] = 0.0f;
        }
    }

    orbit_filter.P_orbit_log_frobenius = mat_log_frobenius(orbit_filter.P_orbit, 6);
    orbit_filter.of_is_initialized = true;
    orbit_filter.of_init_count++;
    orbit_filter.of_last_propagate_time = get_absolute_time();

    LOG_INFO("Initialized orbit filter! (%d times so far)",
             orbit_filter.of_init_count);
    LOG_DEBUG("[orbit_filter] r_eci = [%.3f, %.3f, %.3f] km", gps.r_eci.x,
              gps.r_eci.y, gps.r_eci.z);
    LOG_DEBUG("[orbit_filter] v_eci = [%.6f, %.6f, %.6f] km/s", gps.v_eci.x,
        ...
        AttitudeFilter attitude)
{
    ...
    if (!orbit_filter.of_last_propagate_time)
    {
        orbit_filter.of_last_propagate_time = get_absolute_time();
        return;
    }
    float dt =
        absolute_time_diff_us(orbit_filter.of_last_propagate_time, current_time) *
        1e-6f;
    orbit_filter.of_last_propagate_time = current_time;

    // Propagate state using RK4 with IMU acceleration
    float x[6] = {gps.r_eci.x, gps.r_eci.y, gps.r_eci.z,
                  gps.v_eci.x, gps.v_eci.y, gps.v_eci.z};
    float x_new[6];
    rk4_step(x_new, x, dt, imu.a_body, attitude.quat_);

    // Write back to slate
    gps.r_eci = {x_new[0], x_new[1], x_new[2]};
    gps.v_eci = {x_new[3], x_new[4], x_new[5]};

    // Compute Jacobian ∂a/∂r for covariance propagation
    float r_mag = length(gps.r_eci);
    float r_squared = r_mag * r_mag;
    float r_cubed = r_mag * r_squared;
    float3x3 rrt = outerprod(gps.r_eci, gps.r_eci);
    float3x3 da_dr =
    ...
    mat_mul_square(F, orbit_filter.P_orbit, FP, 6);
    mat_mul_square(orbit_filter.P_orbit, F_T, PF_T, 6);
    ...
    for (int i = 0; i < 36; i++)
    {
        orbit_filter.P_orbit[i] = orbit_filter.P_orbit[i] + (P_dot[i] + Q[i]) * dt;
    }

    orbit_filter.P_orbit_log_frobenius = mat_log_frobenius(orbit_filter.P_orbit, 6);

    LOG_DEBUG("[orbit] r_eci = [%.3f, %.3f, %.3f] km", gps.r_eci.x,
              gps.r_eci.y, gps.r_eci.z);
        ...
        orbit_filter_t &orbit_filter)
{
    ...
    float3 r_ecef = lla_to_ecef(gps.gps_lat, gps.gps_lon, gps.gps_alt);
    float3 r_meas = ecef_to_eci(r_ecef, gps.MJD);
    ...
    float3 lla = {gps.gps_lat, gps.gps_lon, gps.gps_alt};
    float3 v_ecef = enu_to_ecef(v_enu, lla);
    float3 v_meas = ecef_to_eci(v_ecef, gps.MJD);
    ...
    float K[6 * 6];
    mat_add(orbit_filter.P_orbit, R, P_plus_R, 6, 6);
    mat_inverse(P_plus_R, P_plus_R_inv, 6);
    mat_mul_square(orbit_filter.P_orbit, P_plus_R_inv, K, 6);

    // Compute innovation: y = z_measured - x_predicted
    float innovation[6] = {
        r_meas.x - gps.r_eci.x, r_meas.y - gps.r_eci.y,
        r_meas.z - gps.r_eci.z, v_meas.x - gps.v_eci.x,
        v_meas.y - gps.v_eci.y, v_meas.z - gps.v_eci.z,
    };
    ...
    gps.r_eci.x += K_innovation[0];
    gps.r_eci.y += K_innovation[1];
    gps.r_eci.z += K_innovation[2];
    gps.v_eci.x += K_innovation[3];
    gps.v_eci.y += K_innovation[4];
    gps.v_eci.z += K_innovation[5];

    // Update covariance: P_new = (I - K*H)*P = (I - K)*P (since H = I)
    float I_minus_K[6 * 6];
    mat_sub(identity6x6, K, I_minus_K, 6, 6);
    float P_new[6 * 6];
    mat_mul_square(I_minus_K, orbit_filter.P_orbit, P_new, 6);

    for (int i = 0; i < 36; i++)
    {
        orbit_filter.P_orbit[i] = P_new[i];
    }
        ...
        AttitudeFilter attitude)
{
    ...
    gps.gps_lat = 0.0f;    // degrees (equator)
    gps.gps_lon = 0.0f;    // degrees (prime meridian)
    gps.gps_alt = 0.0f;    // km (sea level)
    gps.gps_speed = 0.0f;  // stationary
    gps.gps_course = 0.0f; // doesn't matter
    gps.MJD = 60000.0f;

    orbit_filter_init(gps, imu, orbit_filter);

    LOG_INFO("Initialized at equator (stationary on ground, sea level)");
    LOG_INFO("Initial r_eci: [%.3f, %.3f, %.3f] km (mag=%.3f)", gps.r_eci.x,
             gps.r_eci.y, gps.r_eci.z, length(gps.r_eci));
    LOG_INFO("Initial v_eci: [%.6f, %.6f, %.6f] km/s (mag=%.6f)",
             gps.v_eci.x, gps.v_eci.y, gps.v_eci.z,
             length(gps.v_eci));

    float r_initial_mag = length(gps.r_eci);
    float v_initial_mag = length(gps.v_eci);
    ...
    {
        orbit_filter.of_last_propagate_time =
            get_absolute_time() - static_cast<uint64_t>(dt * 1e6);

        orbit_filter_propagate(gps, imu, orbit_filter);

        // Check for NaN
        if (std::isnan(gps.r_eci.x) || std::isnan(gps.v_eci.x))
        {
    ...
    float r_final_mag = length(gps.r_eci);
    float v_final_mag = length(gps.v_eci);

    LOG_INFO("Final r_eci: [%.3f, %.3f, %.3f] km (mag=%.3f)", gps.r_eci.x,
             gps.r_eci.y, gps.r_eci.z, r_final_mag);
    LOG_INFO("Final v_eci: [%.6f, %.6f, %.6f] km/s (mag=%.6f)", gps.v_eci.x,
             gps.v_eci.y, gps.v_eci.z, v_final_mag);
        ...
        AttitudeFilter attitude)
{
    ...
    gps.gps_lat = 0.0f;   // degrees (equator for simplicity)
    gps.gps_lon = 0.0f;   // degrees
    gps.gps_alt = 400.0f; // km
    gps.MJD = 60000.0f;   // Some MJD value

    // Compute position from GPS
    float3 r_ecef = lla_to_ecef(gps.gps_lat, gps.gps_lon, gps.gps_alt);
    float3 r_eci_init = ecef_to_eci(r_ecef, gps.MJD);
    ...
    gps.gps_speed = v_horizontal / 0.000514444f; // Convert km/s to knots
    gps.gps_course =
        atan2f(v_eci_init.x, v_eci_init.y) * RAD_TO_DEG; // North = 0
    if (gps.gps_course < 0)
        gps.gps_course += 360.0f;
    ...
    float r_initial = length(gps.r_eci);
    float v_initial = length(gps.v_eci);
    ...
    LOG_INFO("Final P log frobenius: %.6f", orbit_filter.P_orbit_log_frobenius);
    LOG_INFO("Final r_eci: [%.3f, %.3f, %.3f] km", gps.r_eci.x,
             gps.r_eci.y, gps.r_eci.z);
    LOG_INFO("Final v_eci: [%.6f, %.6f, %.6f] km/s", gps.v_eci.x,
             gps.v_eci.y, gps.v_eci.z);
        ...
        AttitudeFilter attitude)
{
    ...
    float v_horizontal =
        sqrtf(v_eci_init.x * v_eci_init.x + v_eci_init.y * v_eci_init.y);
    gps.gps_speed = v_horizontal / 0.000514444f;
    gps.gps_course = atan2f(v_eci_init.x, v_eci_init.y) * RAD_TO_DEG;
    if (gps.gps_course < 0)
        gps.gps_course += 360.0f;
    ...
    LOG_INFO("Initial r: %.3f km, v: %.6f km/s", length(gps.r_eci),
             length(gps.v_eci));
    ...
    for (int i = 0; i < steps; i++)
    {
        ...
        float r_mag_current = length(gps.r_eci);
        ...
        if (i > 0 && i % static_cast<int>(orbital_period) == 0)
        {
            ...
            LOG_INFO("Orbit %d complete: r=%.3f km, v=%.6f km/s", orbit_num + 1,
                     r_mag_current, length(gps.v_eci));
        }
    }

    float r_final = length(gps.r_eci);
    float v_final = length(gps.v_eci);
        ...
        AttitudeFilter attitude)
{
    ...
    float x_true[6] = {gps.r_eci.x, gps.r_eci.y, gps.r_eci.z,
                       gps.v_eci.x, gps.v_eci.y, gps.v_eci.z};
    ...
    for (int i = 0; i < steps; i++)
    {
        ...
        gps.MJD = 60000.0f + (i * dt) / 86400.0f;
        ...
        rk4_step(x_true_new, x_true, dt, a_imu_zero, attitude.quat_);
        for (int j = 0; j < 6; j++)
        ...
        if (i % gps_update_interval == 0)
        {
            ...
            gps.gps_course = atan2f(v_enu_meas.x, v_enu_meas.y) * RAD_TO_DEG;
            if (gps.gps_course < 0)
                gps.gps_course += 360.0f;

            orbit_filter_update(gps, imu, orbit_filter);
        }
        ...
        if (i > 0 && i % static_cast<int>(orbital_period) == 0)
        {
            ...
            LOG_INFO("Orbit %d complete: r=%.3f km, v=%.6f km/s, P_log=%.3f",
                     orbit_num + 1, r_mag_current, length(gps.v_eci),
