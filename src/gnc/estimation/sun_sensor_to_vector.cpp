...
int valid_sensors(sun_sensor_data_t *sun_sensor, bool *exclude_sensors);
bool ransac_sun_vector(sun_sensor_data_t *sun_sensor, bool *exclude_sensors,
...
bool sun_sensors_to_vector(sun_sensor_data_t *sun_sensor, float3 *sun_vector_body) {
    ...
    int valid_count = valid_sensors(sun_sensor, exclude_sensors);
...
int valid_sensors(sun_sensor_data_t *sun_sensor, bool *exclude_sensors)
{
    ...
    {
        uint16_t intensity = sun_sensor->intensities[i];
    ...
    for (int i = 0; i < NUM_SUN_SENSOR_OPPOSITE_PAIRS; i++)
    {
        ...
        uint16_t intensity1 = sun_sensor->intensities[s1];
        uint16_t intensity2 = sun_sensor->intensities[s2];
                       ...
                       float3 &best_sun_vector, int &best_inlier_count)
{
         ...
         iter++)
    {
        ...
        for (int i = 0; i < 3; i++)
        {
            ...
            b[i] =
                static_cast<float>(sun_sensor->intensities[sensor_idx]);
        }
