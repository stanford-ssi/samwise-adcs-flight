...
linalg::vec<float, 3> compute_control_torque(
    ...
    linalg::vec<float, 3> omega_current,
    linalg::vec<float, 3> omega_desired)
{
    ...
    linalg::vec<float, 4> diff_quat = linalg::qmul(q_current, linalg::qconj(q_desired));
...
void test_reaction_wheel_allocation() 
{
    ...
    // Test the function
    auto torque = compute_control_torque(q_current, q_desired, 
                                       omega_current, omega_desired);
