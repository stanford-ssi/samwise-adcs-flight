...
// -- RX --
bool pio_uart_rx_init(PIO *pio_rx, uint *sm_rx, uint8_t pin, uint32_t baud)
{
...
uint8_t pio_uart_getc(PIO pio_rx, uint sm_rx) 
{
    return uart_rx_program_getc(pio_rx, sm_rx);;
}

// -- TX --
bool pio_uart_tx_init(PIO *pio_tx, uint *sm_tx, uint8_t pin_tx, uint32_t baud)
{
