 ...
 *
 * @param mag_field_body Pointer to float3 to store the calibrated,
 * body-frame magnetic field UNIT vector [unitless]
 * @param mag_field_raw Pointer to float3 to store the uncalibrated
 * SENSOR-frame magnetic field [microTesla]
 * @return rm3100_error_t Error code (RM3100_OK on success)
 */
rm3100_error_t rm3100_get_reading(float3 *mag_field_body, float3 *mag_field_raw)
{
    ...
    // 1. Scale into microTesla, still in the SENSOR frame.
    //
    // mag_field_raw is deliberately left in sensor axes and uncalibrated: it
    // is what scripts/calibration/magnetometer_calibration.py fits against,
    // so applying any correction here would invalidate the constants it
    // produces.
    *mag_field_raw = {raw_x_lsb * lsb_to_uT, raw_y_lsb * lsb_to_uT,
                      raw_z_lsb * lsb_to_uT};

    LOG_DEBUG("[rm3100] b_sensor_raw = [%.6f, %.6f, %.6f] uT",
              mag_field_raw->x, mag_field_raw->y, mag_field_raw->z);

    // 2. Apply calibration to get the true field, compensating for the
    // magnetic fields generated by the electronics on the spacecraft. Still in
    // the sensor frame - the calibration was fit there.
    ...
    *mag_field_body = normalize(field_body);

    LOG_DEBUG("[rm3100] b_body = [%.6f, %.6f, %.6f]", mag_field_body->x,
              mag_field_body->y, mag_field_body->z);
