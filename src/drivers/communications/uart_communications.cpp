...
static inline uint16_t rx_count(uint8_t idx) {
    return (rx_head[idx] - rx_tail[idx] + UART_RX_BUFFER_SIZE) % UART_RX_BUFFER_SIZE;
}

static inline uint16_t tx_count(uint8_t idx) {
    return (tx_head[idx] - tx_tail[idx] + UART_TX_BUFFER_SIZE) % UART_TX_BUFFER_SIZE;
}
...
void uart_comms_init(uart_inst_t *uart_instance,
        uint8_t tx,
        uint8_t rx,
        uint32_t baud) {
...
uint16_t uart_comms_tx(uart_inst_t *uart_instance,
        uint8_t *data,
        uint16_t length) {
...
uint16_t uart_comms_get_packet(uart_inst_t *uart_instance,
        uint8_t *buffer,
        uint16_t max_length) {
