...
static bool init_drivers(motor_slate_t *motor_slate)
{
    adm1176_t power_monitor =
        adm1176_mk(SAMWISE_POWER_MONITOR_I2C, ADM1176_I2C_ADDR,
...
{
    motor_slate->motors[0] = motor_init(
        MOTOR_0_FAULT_PIN, MOTOR_0_SLEEP_PIN, MOTOR_0_DRVOFF_PIN,
        MOTOR_0_SCS_PIN, MOTOR_0_BRAKE_PIN, MOTOR_0_PWM_PIN, MOTOR_0_FGOUT_PIN);

    motor_slate->motors[1] = motor_init(
        MOTOR_1_FAULT_PIN, MOTOR_1_SLEEP_PIN, MOTOR_1_DRVOFF_PIN,
        MOTOR_1_SCS_PIN, MOTOR_1_BRAKE_PIN, MOTOR_1_PWM_PIN, MOTOR_1_FGOUT_PIN);

    motor_slate->motors[2] = motor_init(
        MOTOR_2_FAULT_PIN, MOTOR_2_SLEEP_PIN, MOTOR_2_DRVOFF_PIN,
        MOTOR_2_SCS_PIN, MOTOR_2_BRAKE_PIN, MOTOR_2_PWM_PIN, MOTOR_2_FGOUT_PIN);

    motor_slate->motors[3] = motor_init(
        MOTOR_3_FAULT_PIN, MOTOR_3_SLEEP_PIN, MOTOR_3_DRVOFF_PIN,
        MOTOR_3_SCS_PIN, MOTOR_3_BRAKE_PIN, MOTOR_3_PWM_PIN, MOTOR_3_FGOUT_PIN);

    for (int i = 0; i < 4; i++)
    {
        motor_t *motor = &motor_slate->motors[i];
        gpio_set_irq_enabled_with_callback(motor->FGOUT_pin_,
                                           GPIO_IRQ_EDGE_FALL, true, &gpio_irq);
        gpio_set_irq_enabled_with_callback(motor->FAULT_pin_,
                                           GPIO_IRQ_EDGE_FALL, true, &gpio_irq);
    }
...
static bool init_uart(motor_slate_t *motor_slate)
{
    ...
    */
    ...
    // Initialise UART 1
    gpio_set_irq_enabled_with_callback(ADCS_UART_COMM_RX, GPIO_IRQ_EDGE_FALL,
                                       true, &gpio_irq);
    motor_slate->adcs_uart =
        software_uart_init(ADCS_UART_COMM_RX, ADCS_UART_COMM_TX);
