...
void ekf_test(slate_t *slate)
{
    ...
    LOG_INFO(
        "[gnc] actual sun vector [x,y,z] in body frame is: %.6f, %.6f, %.6f",
        slate->sun_vector_body[0], slate->sun_vector_body[1],
        slate->sun_vector_body[2]);
    LOG_INFO(
        "[gnc] expected sun vector [x,y,z] in body frame is: %.6f, %.6f, %.6f",
        S_local_expected[0], S_local_expected[1], S_local_expected[2]);
    LOG_INFO("[gnc] actual B field vector [x,y,z] in body frame is: %.6f, "
             "%.6f, %.6f",
             slate->b_field_local[0], slate->b_field_local[1],
             slate->b_field_local[2]);
    LOG_INFO("[gnc] expected B field vector [x,y,z] in body frame is: %.6f, "
             "%.6f, %.6f",
...
{
    // Calculate Expected quternion when +X axis points to North and +Z axis
    // points up
    ...
    const float ang_y =
        (37.42f) * DEG_TO_RAD; // rotation angle around y: lattitude
    const float ang_z =
        GMST + (180.0f - 122.17f) * DEG_TO_RAD; // rotation angle around z: GMST
    ...
    // Attitude Matrix
    const float R[3 * 3] = {Cy * Cz, Cy * Sz,  Sy,       -Sz, Cz,
                            0.0f,    -Sy * Cz, -Sy * Sz, Cy};
