...
def plot_magnetic_field_vectors(port='/dev/tty.usbmodem101', baudrate=115200):
    ...
    plt.title('Magnetic Field Vector Visualization', color='white', fontsize=14, weight='bold')

    plt.ion()
    plt.show(block=False)

    pattern = r'\[DEBUG\].*\[sensors\] Magnetometer reading: \[([-\d.]+), ([-\d.]+), ([-\d.]+)\]'
            ...
            try:
                ...
                if match:
                    ...
                    if magnitude > 1e-6:  # Non-zero vector
                        # Normalize the vector to unit length for visualization
                        latest_vector = (x/magnitude, y/magnitude, z/magnitude)
                ...
                if latest_vector and (current_time - last_plot_time) >= plot_interval:
                    ...
                    x_norm, y_norm, z_norm = latest_vector

                    # Create new arrow
                    current_arrow = ax.quiver(0, 0, 0, x_norm, y_norm, z_norm,
                                            color='#FF6B35',  # Orange color for magnetic field
                                            linewidth=4,
...
if __name__ == "__main__":
    plot_magnetic_field_vectors(port='/dev/tty.usbmodem101')